...
class DataList extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.selectedId) {
      var selected_item = nextProps.options.filter((value) => value[nextProps.id] == nextProps.selectedId)[0];
      if (selected_item != undefined) {
        this.setState({ selectedOptionId: selected_item[nextProps.id], inputFieldText: selected_item.display_name });
      }
    }
    else {
