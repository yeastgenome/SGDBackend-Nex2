...
from src.data_helpers.data_helpers import combine_panther_locus_data, pair_pantherid_to_sgdids, get_output, get_locus_alias_data, get_pers_output
...
#                    LocusAlias.locus_id == item.dbentity_id).all()
               ...
               #     #      obj["basicGeneticEntity"]["genomeLocations"][0][["assembly"] = dna_seq_annotation_obj[0].genome_release
                ...
                if dna_seq_annotation_obj[0].so.so_id == 263757:  #change ORF to gene SO ID
                    obj["gene_type"] = "SO:0001217"
                else:
                    obj["gene_type"] = dna_seq_annotation_obj[0].so.soid
...
# Add name_description if it exists
           ...
           #         obj["name"] = item.name_description
            ...
            if (len(result) > 0):
                ...
                file_name = 'data/SGD' + SUBMISSION_VERSION + 'bgiPersistent.json'
