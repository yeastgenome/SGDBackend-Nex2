...
def get_allele_information():
    ...
    alleleObjList = DBSession.execute(
        ...
        f"s.display_name, s.format_name, db.date_created "
        f"from nex.alleledbentity ad "
        f"inner join nex.dbentity db on ad.dbentity_id = db.dbentity_id "
        f"inner join nex.so s on ad.so_id = s.so_id ").fetchall()
    ...
    if (len(alleleObjList) > 0):
        ...
        try:
            for alleleObj in alleleObjList:
                ...
                allele_alias_list = DBSession.query(AlleleAlias).filter(and_(AlleleAlias.allele_id == alleleObj[0], AlleleAlias.alias_type != 'SGD Secondary')).all()
