        ...
        try:
            for alleleObj in alleleObjList:
                ...
                alleleSynList = DBSession.execute(
                    ...
                    f"from nex.alleledbentity ad "
                    f"inner join nex.allele_alias aa on ad.dbentity_id = aa.allele_id "
                    f"where ad.dbentity_id ="+ str(alleleObj[0])).fetchall()
                if alleleSynList:
                    obj['synonyms'] = []
                    for x in alleleSynList:
                        if x not in obj['synonyms'] and x is not None:
                            obj['synonyms'].append(str(x[0]))
