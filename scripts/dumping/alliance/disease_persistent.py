...
def get_disease_association_data():
            ...
            if uniqkey in list(result.keys()):
                ...
                if item.taxonomy_id and item.taxonomy.display_name != 'Saccharomyces cerevisiae':
                    ...
                    strainObj = DBSession.query(Straindbentity).filter(
                        Straindbentity.taxonomy_id == item.taxonomy_id).one()
                    obj["sgd_strain_background_identifier"] = "SGD:" + \
                        strainObj.sgdid
    ...
    if len(list(result.keys())) > 0:
        ...
        output_obj = get_pers_output(
            SUBMISSION_TYPE, list(result.values()), LINKML_VERSION)
