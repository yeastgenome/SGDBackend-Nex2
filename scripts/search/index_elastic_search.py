...
class Index(object):
    '''
    Deals with all aspects of creating and deleting ES indexes
    '''
    def __init__(self):
        self.INDEX_NAME = os.environ.get("ES_INDEX_NAME", "searchable_items_aws")
        self.DOC_TYPE = "searchable_item"
        self.ES_URI = os.environ["WRITE_ES_URI"]
        self.es = Elasticsearch(self.ES_URI, retry_on_timeout=True)

    def delete_mapping(self):
        print("Deleting mapping...")
        response = requests.delete(self.ES_URI + self.INDEX_NAME + "/")
    ...
    def put_mapping(self):
        ...
        response = requests.put(self.ES_URI + self.INDEX_NAME + "/", json=mapping)
        if response.status_code != 200:
            print("ERROR: " + str(response.json()))
        else:
            print("SUCCESS")

    def create_index(self, body, id):
        """
        Index the data specified in body with the given id

        Args:
            body (dictionary): data to be indexed
            id (string): unique ID for this index

        Returns:
            None
        """
        self.es.index(index=self.INDEX_NAME, doc_type=self.DOC_TYPE, body=body, id=id)

    def bulk_index(self, body):
        """
        Index data in bulk numbers

        Args:
            body (List): data to be indexed in list format
    ...
    def print_index_msg(self, count, category):
        """
        Print indexing message for the category that is being indexed

        Args:
            count (int): number of data items of this category being indexed
            category (string): category being indexed

        Returns:
            None
        """
        print("Indexing {} {}".format(count, category))

    def create_bulk_data_index(self, id):
        """
        Create the index header for indexing data in bulk

        Args:
            id (string): Unique Identifier for the index

        Returns:
            Dictionary containing a single key called "index" representing the
    ...
    def index_toolbar_links(self):
        ...
        self.print_index_msg(len(links), "toolbar links")
        ...
        for l in links:
            ...
            self.create_index(obj, l[1])


    def index_colleagues(self):
        ...
        self.print_index_msg(len(colleagues), "colleagues")
        bulk_data = []
    ...
    def index_observables(self):
        observables = DBSession.query(Apo).filter_by(apo_namespace="observable").all()
        self.print_index_msg(len(observables), "observables")
        bulk_data = []
    ...
    def index_strains(self):
        strains = DBSession.query(Straindbentity).all()
        self.print_index_msg(len(strains), "strains")
        for strain in strains:
            ...
            self.create_index(obj, str(uuid.uuid4()))


    def index_reserved_names(self):
        # only index reservednames that do not have a locus associated with them
        reserved_names = DBSession.query(Reservedname).all()
        self.print_index_msg(len(reserved_names), "reserved names")
    ...
    def index_genes(self):
        ...
        bulk_data = []
        self.print_index_msg(len(all_genes), "genes")
    ...
    def index_not_mapped_genes(self):
        ...
        with open("./scripts/search/not_mapped.json",
            ...
            self.print_index_msg(len(_data), "not physically mapped genes")
    ...
    def index_downloads(self):
        ...
        self.print_index_msg(len(files), "download files")
    ...
    def index_complex_names(self):
        complexes = DBSession.query(Complexdbentity).all()
        self.print_index_msg(len(complexes), "complex names")
        bulk_data = []
    ...
    def load_go_id_blacklist(self, list_filename):
        go_id_blacklist = set()
    ...
    def index_disease_terms(self):
        dos = DBSession.query(Disease).all()
        self.print_index_msg(len(dos), "DO terms")
        bulk_data = []
    ...
    def index_references(self):
        ...
        self.print_index_msg(len(_references), "references")
        bulk_data = []
    ...
    def index_chemicals(self):
        ...
        for item_key, item_v in _result.items():
            if item_v is not None:
                ...
                bulk_data.append(self.create_bulk_data_index("chemical_" + str(item_key)))
                bulk_data.append(obj)
                if len(bulk_data) == 300:
                    self.bulk_index(bulk_data)
                    bulk_data = []
    ...
    def setup(self):
        ...
        index_exists = self.INDEX_NAME in indices
        if not index_exists:
            self.put_mapping()


def index_part_1(obj):
    obj.index_toolbar_links()
    obj.index_strains()
