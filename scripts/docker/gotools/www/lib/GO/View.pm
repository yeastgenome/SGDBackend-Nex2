...
use warnings;
use GD;
use GD::Polyline;   # we use polylines for edges -- mark
use GraphViz;
use IO::File;
...
#########################################################################
	...
	$self->{PVALUE_CUTOFF} = $args{-pvalueCutOff} || 0.01;
        ...
        #LINDA'S ADDITION: 
#        $self->{MAX_TOP_NODE_TO_SHOW} = $args{-maxTopNodeToShow};
        $self->{MAX_CHILD_GOID_NUM} = $args{-maxChildGoidNum};
    ...
    # support these new options -- mark
    $self->{MAKE_PS} = $args{-makePS};
    $self->{MAKE_SVG} = $args{-makeSVG};
...
#########################################################################
    ...
    # support SVG output -- mark
    if (defined $self->{MAKE_SVG} && $self->{MAKE_SVG}){
	my $file = $self->{IMAGE_FILE};
	$file =~ s/\.\w+$/\.svg/;
	my $fh = IO::File->new($file, q{>} )|| die "Cannot create $file : $!";
	print $fh $self->graph->as_svg;
	$fh->close;
    }
...
#    {
#    my $fh = IO::File->new("/tmp/gtf.dot", q{>} )|| die "Cannot create /tmp/gtf.dot : $!";
#    print $fh $graphText;
#    $fh->close;
#    }
    ...
    $graphText =~ s/\\\n//g;
    # replace any newlines not following ';' or '{' with space
    $graphText =~ s/([^\;\{])\n\s*/$1 /g;
#    print $graphText;
...
#    if ($graphText =~ /graph \[bb=\"0,0,([0-9]+),([0-9]+)\" *\]\;/) {
	...
	$width  = $1 * $self->{WIDTH_DISPLAY_RATIO};  # round up?

	$height = $2 * $self->{HEIGHT_DISPLAY_RATIO}; # round up?
	...
	# in case line was continuated
	# sometimes the break is just before ',', in which case the extra space needs to be removed
	$line =~ s/\\(\s)/$1/g;
	$line =~ s/\s,/,/g;
...
#	if ($line =~ / *node[0-9]+ *\[.*(label=\".+\")/i) {
	if ($line =~ /^\s*node[0-9]+\s*\[/) {

	    # it's a node
	    push(@nodeLine, $line);

#	}elsif ($line  =~ / *node[0-9]+ *-> *node[0-9]+ \[pos=\"(.+)\"/i) {
	}elsif ($line  =~ /^\s*node[0-9]+\s*->\s*node[0-9]+\s*\[pos=\"(.+)\"/i) {
...
#########################################################################
    ...
    my $boxW = $1 * 60 + 5;  # tweak - mark

    $line =~ /height=\"?([\d\.]+)/;

    my $boxH = $1 * 55;   # tweak - mark
    ...
    my $x1 = int($1 * $self->{WIDTH_DISPLAY_RATIO}-$boxW/2 + $border);

    my $y1 = int($height - $2 * $self->{HEIGHT_DISPLAY_RATIO} - $boxH/2 + $border + $self->{MOVE_Y});

    my $x2 = $x1 + $boxW;

    my $y2 = $y1 + $boxH;

    # Retrieve stored goid, which we now save in
    # a hash, rather than in the label. -- mark
    my $goid = $self->{LABEL_TO_GOID}->{$label};
    ...
    my $i = 0;
    my $y3 = $y1 + int(($boxH - 12*(@label)) / 2) - 2;  # center - mark
...
#########################################################################
    ...
    # rather than crooked edges, draw splines -- mark
...
#	     $self->{MOVE_Y} + 5;
	...
	$gd->im->polydraw($spline, $gd->black);
    } else {
	$gd->im->polydraw($polyline, $gd->black);
    }

# this is the original routine -- mark
#    my @point = split(/ /, $line);
#
...
#########################################################################
    ...
    my $stdGoid;  # moved up in scope -- mark
...
# See below for why this was changed. -- mark
...
#	    $label = $self->_processLabel($label)."\n".$stdGoid;
    ...
    if ($goid eq $self->_goid) {
	# remove the underscore from the root term
	$label =~ s/_/ /;
    }
   
# As seen above, previously the goid was stored in the
# node label, which then appeared in SVG output.
# Rather than storing information in the label,
# we save it in a hash. -- mark
#! this assumes all labels will be unique
    my $modlabel = $label;
    $modlabel =~ s/\n/\\n/g;
    $self->{LABEL_TO_GOID}->{$modlabel} = $stdGoid;

    # now add the node to the graph, with the appropriate label
    
    # helvetica 15 closely matches gdSmallFont
...
#    my $maxTopNodeToShow = $self->{MAX_TOP_NODE_TO_SHOW};
    my $maxChildGoidNum = $self->{MAX_CHILD_GOID_NUM};
...
#		push(@directAnnotatedGoid, $goid);
		...
		if ((defined($maxChildGoidNum)) && (scalar(@directAnnotatedGoid) < $maxChildGoidNum)) {
		    push(@directAnnotatedGoid, $goid);
		}
...
##########################################################################
	...
	}elsif ($name =~ /^makeSvg/i){   # add SVG support -- mark
...
#! Different packages have different ideas of what these named
#! colors are. For example, GraphViz and GD do not agree. It
#! would probably be prudent to use RGB values instead of names.
#! -- mark
