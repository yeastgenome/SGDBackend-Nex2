...
=head1 SYNOPSIS
    ...
    my $numRows = $report->print(pvalues  => \@pvalues,
                                 ...
                                 fh       => \*HTML,
                                 cutoff   => 0.01,
                                 geneUrl  => 'https://www.yeastgenome.org/locus/<REPLACE_THIS>',
                                 goidUrl  => 'https://www.yeastgenome.org/go/GO:0000916=<REPLACE_THIS>');
...
######################################################################################
    ...
    my $annoFile = $args{'annoFile'};
    my $correction = $args{'correction'};  # -- mark
    $correction = "bonferroni" if (!$correction);
    my $replacementText = "<REPLACE_THIS>";
    my $replacementText2 = "<REPLACE_DBID>";  # added because some sites may not allow gene name - mark
	...
	# this is the same as the uncorrected PVALUE if no correction method used -- mark
	...
	foreach my $databaseId (keys %{$pvalue->{ANNOTATED_GENES}}){
		...
		$url =~ s/$replacementText2/$databaseId/;
		$gene = a({-href=>$url,
			   -target=>'infowin'}, $gene);
...
#	my $frequency = $pvalue->{NUM_ANNOTATIONS}." out of $numGenes genes, $frequencyPercent\%";
	my $frequency = $pvalue->{NUM_ANNOTATIONS}." of $numGenes genes, $frequencyPercent\%";
...
#	my $genomeFrequency = $pvalue->{TOTAL_NUM_ANNOTATIONS}." out of $totalNum genes, $geneFrequencyPercent\%";
	my $genomeFrequency = $pvalue->{TOTAL_NUM_ANNOTATIONS}." of $totalNum genes, $geneFrequencyPercent\%";
    ...
    $self->_printTable($fh, $rows, $aspect, $annoFile, $cutoff, $hasFdr, $args{evidenceCodes}, $args{evidenceCodesNot}, $correction) if ($numRows > 0);
...
# This method prints out the actual html table
    ...
    my ($self, $fh, $rows, $aspect, $annoFile, $cutoff, $hasFdr, $evidenceCodes, $evidenceCodesNot, $correction) = @_;
    ...
    print $fh "<table align=center border=1 cellpadding=2 width=400>";
#    print $fh table({-align       => 'center',
#		     -border      => 1,
#		     -cellpadding => 2,
#		     -width       => 400},
    print $fh Tr(td({-bgcolor => '#FFCC99',
		     -align   => 'center',
		     -width   => '100%',
		     -nowrap  => undef},
#			  b("Terms from the $aspect Ontology with p-value as good or better than $cutoff"))));
		    b("Terms from the $aspect Ontology ",$annoFile? "of $annoFile " : "", "with p-value <= $cutoff")));

    if ($evidenceCodes) {
	print $fh Tr(td({-bgcolor => '#CCBBAA',
			 -align   => 'center',
			 -width   => '100%',
			 -nowrap  => undef},
			b("Evidence code filter included: ".join(" ", sort { $a cmp $b } keys(%{$evidenceCodes})))));
    } elsif ($evidenceCodesNot) {
	print $fh Tr(td({-bgcolor => '#CCBBAA',
			 -align   => 'center',
			 -width   => '100%',
			 -nowrap  => undef},
			b("Evidence codes filtered out: ".join(" ", sort { $a cmp $b } keys(%{$evidenceCodesNot})))));
    }

    my $headings = th({-align => 'center'}, "Gene Ontology term").
		   th({-align => 'center'}, "Cluster frequency").
#		   th({-align => 'center'}, "Genome frequency of use").
		   th({-align => 'center'}, "Genome frequency");
    if ($correction ne "none") {  # change column header depending on whether correction is enabled -- mark
	$headings .= th({-align => 'center'}, "Corrected<br>P-value");
    } else {
	$headings .= th({-align => 'center'}, "Uncorrected<br>P-value");
    }
	...
	$headings .= th({-align => 'center'}, "FDR").
	    th({-align => 'center'}, "False<br>Positives");
